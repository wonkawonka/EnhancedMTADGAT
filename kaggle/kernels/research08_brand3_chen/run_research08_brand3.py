...
def find_root():
 for p in Path("/kaggle/input").rglob("battery_brand3"):
  if (p/"label").is_dir() and any((p/x).is_dir() for x in ("train","test","data")): return p.parent
 raise FileNotFoundError("battery_brand3")
