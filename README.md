...
这是一个面向多变量时序与电池异常检测的 `MTAD-GAT` 扩展项目。第三章当前冻结为简化 C3：全通道统计 Token、prototype-query 交叉注意力、输出拼接和融合层 FiLM；99 号起进入正式多 seed/五折实验，97/98 仅作开发归档。第四章研究与主干解耦的电-热响应一致性分支。
