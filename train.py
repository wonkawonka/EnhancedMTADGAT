...
def train_multi_entities(args):
    """
    为所有实体训练模型
    """
    if args.dataset != 'CALCE':
        raise ValueError("Multi-entity training is only supported for CALCE dataset")
    
    # 获取所有实体
    entity_names = get_all_calce_entities()
    print(f"Found {len(entity_names)} entities: {entity_names}")
    
    # 为每个实体训练模型
    for entity_name in entity_names:
        print(f"\nTraining model for entity: {entity_name}")
        try:
            train_single_entity(entity_name, args)
        except Exception as e:
            print(f"Error training entity {entity_name}: {e}")
            import traceback
            traceback.print_exc()
