...
车辆级报告统一把 PR-AUC 定义为 Average Precision（`average_precision_score`，并保留 `average_precision`/`auprc` 同值别名）；梯形积分另记为 `pr_auc_trapezoid`，不作为主指标。Brand3 主表统一遵循 Zhang et al. 的 DyAD 协议：带标签 calibration 在 5%–95% 搜索 Top-p，并按公开 notebook 的片段排序规则选择阈值；测试折标签不参与这两个选择。旧严格正常校准、固定 Top-5% 输出仅作开发历史。验证集阈值 F1 只作辅助，不能用窗口级 F1 代替。
