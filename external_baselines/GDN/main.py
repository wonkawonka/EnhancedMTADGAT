...
PROJECT_PROCESSED_ROOTS = [
    PROJECT_ROOT / 'datasets' / 'data' / 'processed',
...
def resolve_project_processed_paths(dataset_name):
    normalized = str(dataset_name).strip()
    lower_name = normalized.lower()
    candidate_stems = []
    if lower_name == 'smap':
        candidate_stems.append('SMAP')
    elif lower_name == 'msl':
        candidate_stems.append('MSL')
    elif lower_name == 'smd':
        candidate_stems.append('machine-1-1')
    candidate_stems.extend([normalized, normalized.upper(), normalized.lower()])

    for root in PROJECT_PROCESSED_ROOTS:
        for stem in candidate_stems:
            train_path = root / f'{stem}_train.pkl'
            test_path = root / f'{stem}_test.pkl'
            label_path = root / f'{stem}_test_label.pkl'
            if train_path.exists() and test_path.exists() and label_path.exists():
                return train_path, test_path, label_path, stem
    return None, None, None, None


def build_fc_graph_from_features(feature_map):
...
def load_project_processed_frames(dataset_name):
    ...
    test_df = pd.DataFrame(test, columns=feature_map)
    test_df['attack'] = labels.astype(int)
    fc_struc = build_fc_graph_from_features(feature_map)
    print(f'Loading {dataset_name} from project processed files: {resolved_stem}')
    return train_df, test_df, feature_map, fc_struc
...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        train_csv_path = Path(f'./data/{dataset}/train.csv')
        test_csv_path = Path(f'./data/{dataset}/test.csv')
        project_processed_data = None
        if train_csv_path.exists() and test_csv_path.exists():
            train_orig = pd.read_csv(str(train_csv_path), sep=',', index_col=0)
            test_orig = pd.read_csv(str(test_csv_path), sep=',', index_col=0)
            feature_map = get_feature_map(dataset)
            fc_struc = get_fc_graph_struc(dataset)
        else:
            project_processed_data = load_project_processed_frames(dataset)
            if project_processed_data is None:
                raise FileNotFoundError(f'No csv data or project processed data found for dataset: {dataset}')
            train_orig, test_orig, feature_map, fc_struc = project_processed_data
