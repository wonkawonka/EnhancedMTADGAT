...
class ProjectProcessedLoader(object):
    def __init__(self, dataset_name, data_path, win_size, step, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()
        ...
        if dataset_name in dataset_map:
            proc_dir = dataset_map[dataset_name]
            stem = dataset_name
        elif str(dataset_name).startswith("SMAP"):
            proc_dir = project_root / "datasets" / "data" / "processed"
            stem = "SMAP"
        elif str(dataset_name).startswith("MSL"):
            proc_dir = project_root / "datasets" / "data" / "processed"
            stem = "MSL"
        ...
        test_labels = np.asarray(test_labels, dtype=np.float64).squeeze()
    ...
    def __getitem__(self, index):
        ...
        if len(labels) < self.win_size:
            labels = self.test_labels[-self.win_size:]
        return np.float32(arr[index:index + self.win_size]), np.float32(labels)
...
def get_loader_segment(index, data_path, batch_size, win_size=100, step=100, mode='train', dataset='KDD'):
    ...
    elif any(dataset.startswith(p) for p in ['SMAP', 'MSL', 'BMS_', 'NASA_']):
        dataset = ProjectProcessedLoader(dataset, data_path, win_size, 1, mode)
