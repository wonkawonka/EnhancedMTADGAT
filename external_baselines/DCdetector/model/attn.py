...
class DAC_structure(nn.Module):
    ...
    def forward(self, queries_patch_size, queries_patch_num, keys_patch_size, keys_patch_num, values, patch_index, attn_mask):
        ...
        # Upsampling
        series_patch_size = series_patch_size.repeat_interleave(self.patch_size[patch_index], dim=2).repeat_interleave(self.patch_size[patch_index], dim=3)
        ...
        series_patch_size = series_patch_size.reshape(-1, self.channel, *series_patch_size.shape[1:]).mean(dim=1)
        series_patch_num = series_patch_num.reshape(-1, self.channel, *series_patch_num.shape[1:]).mean(dim=1)
