...
class DCdetector(nn.Module):
    ...
    def forward(self, x):
        ...
        for patch_index, patchsize in enumerate(self.patch_size):
            ...
            x_patch_size = x_patch_size.permute(0, 2, 1)
            x_patch_num = x_patch_num.permute(0, 2, 1)

            b_size, m_size, l_size = x_patch_size.shape
            n_patch = l_size // patchsize
            x_patch_size = x_patch_size.reshape(b_size, m_size, n_patch, patchsize).reshape(-1, n_patch, patchsize)
            x_patch_size = self.embedding_patch_size[patch_index](x_patch_size)
            x_patch_num = x_patch_num.reshape(b_size, m_size, patchsize, n_patch).reshape(-1, patchsize, n_patch)
        ...
        series_patch_mean = [item for sublist in series_patch_mean for item in sublist]
        prior_patch_mean = [item for sublist in prior_patch_mean for item in sublist]
