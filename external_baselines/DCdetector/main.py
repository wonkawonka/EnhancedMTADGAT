...
if __name__ == '__main__':
    ...
    parser.add_argument('--patch_size', type=parse_patch_size, default=[5])
    ...
    if config.output_dir:
        ...
        config.model_save_path = str(output_root / "checkpoints")
        log_dir = output_root / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        internal_log_path = log_dir / f"{config.data_path}.log"
    else:
        internal_log_path = Path("result") / f"{config.data_path}.log"
    ...
    sys.stdout = Logger(str(internal_log_path), sys.stdout)
    if config.mode == 'train':
        print("\n\n")
