...
from src.engine.spot import SPOT, dSPOT
...
def adjust_predicts(score, label, threshold, pred=None, calc_latency=False):
    ...
    根据给定的 `score`、`threshold`（或 `pred`）和 `label` 计算调整后的预测标签。
...
def pot_eval(init_score, score, label, q=1e-3, level=0.99, dynamic=False):
    ...
    s.initialize(level=level, min_extrema=False)  # 初始化阈值
...
def bf_search(score, label, start, end=None, step_num=1, display_freq=1, verbose=True):
    ...
    在 [`start`, `end`) 范围内搜索最佳阈值以获得最佳 F1 分数。
...
def find_epsilon(errors, reg_level=1):
    ...
    Hundman 等人提出的阈值方法（https://arxiv.org/abs/1802.04431）。
