...
from src.data.nc_battery import StreamingMinMaxScaler, build_index, load_snippet, split_vehicle_folds
...
def load_external_protocol_data(
    dataset: str, *, brand_fold: int = 0, seed: int = 3407, val_ratio: float = 0.1
    ...
    if name == "BRAND3":
        ...
        splits = split_vehicle_folds(records, int(brand_fold), seed=int(seed), protocol="strict_normal_validation")
        train_records = splits["train"]
        validation_records = splits["validation"]
        test_records = splits["test"]
        scaler = StreamingMinMaxScaler().fit_records(train_records)
        ...
        return ExternalProtocolData(
            ...
            metadata={"brand": 3, "fold": int(brand_fold), "protocol": "strict_normal_validation", "normalization": scaler.state_dict(), "seed": int(seed)},
        )
