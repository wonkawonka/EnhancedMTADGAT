...
def merge_score_and_raw(score_df, raw_df, feature_prefix="Raw_"):
    ...
    raw_feature_cols = [c for c in raw_df.columns if c not in {"Segment_ID", "Segment_Pos", "Global_Pos"}]
    raw_df = raw_df.rename(columns={c: f"{feature_prefix}{c}" for c in raw_feature_cols})
    return pd.concat([score_df, raw_df], axis=1)
...
def plot_slice_boxpanels(df, slice_order, output_path, title):
    ...
    axes[0].set_ylabel("A_Score_Global")
    fig.suptitle(title)
    fig.tight_layout()
...
def plot_transition_profiles(profile_df, output_path):
    ...
    ax.set_xlabel("Relative position to phase switch")
    ax.set_ylabel("Mean A_Score_Global")
    ax.set_title("NASA phase-switch score profile")
    ax.grid(alpha=0.25)
...
def plot_bms_branch_contribution(summary_df, output_path):
    if summary_df.empty:
        return
    target = summary_df[summary_df["experiment_label"].isin(["C4+Physics", "C4-StructureOnly"])].copy()
    if target.empty:
        return
    ...
    for ax, slice_name in zip(axes, BMS_SLICE_ORDER):
        ...
        for idx, metric in enumerate(metrics):
            if metric not in slice_df.columns:
                continue
            offset = (idx - 1) * width
            ax.bar(x + offset, slice_df[metric].to_numpy(), width=width, label=metric.replace("_mean", ""))
        ax.set_xticks(x)
    ...
    fig.suptitle("BMS branch contribution by slice")
    fig.tight_layout()
    fig.savefig(output_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def preferred_nasa_experiments(df):
    preferred = ["C3", "C3+PhysEnc", "C3+PhysReg", "C3+Physics"]
...
def preferred_bms_experiments(df):
    preferred = ["C3", "C3+PhysEnc", "C3+PhysReg", "C3+Physics", "C4-StructureOnly", "C4+Physics"]
    available = set(df["experiment_label"].unique())
    ordered = [item for item in preferred if item in available]
    if ordered:
        return ordered
...
def choose_nasa_case_center(df, target_slice, metric):
    ...
    if {"C3", "C3+Physics"}.issubset(set(target_df["experiment_label"].unique())):
        base_df = target_df[target_df["experiment_label"] == "C3"][
            ["entity_name", "Global_Pos", metric]
        ].rename(columns={metric: f"{metric}_base"})
        phys_df = target_df[target_df["experiment_label"] == "C3+Physics"][
            ["entity_name", "Global_Pos", metric]
...
def choose_bms_case_center(df):
    candidates = df.copy()
    if "C4+Physics" in set(candidates["experiment_label"].unique()):
        candidates = candidates[candidates["experiment_label"] == "C4+Physics"].copy()
...
def plot_nasa_case(df, output_path, title, entity_name, center_pos, radius):
    ...
    axes[1].set_ylabel("A_Score")
    axes[1].grid(alpha=0.25)
    axes[1].legend(frameon=False, ncol=2)

    axes[2].plot(x, reference_df["Raw_voltage"].to_numpy(), color="black", linewidth=1.2, label="True Voltage")
    for exp in experiments:
        exp_df = local_df[local_df["experiment_label"] == exp].sort_values("Global_Pos")
        axes[2].plot(exp_df["Global_Pos"], exp_df["Recon_1"], linewidth=1.1, label=f"{exp} Recon")
    axes[2].set_ylabel("Voltage")
    axes[2].grid(alpha=0.25)
    axes[2].legend(frameon=False, ncol=2)

    axes[3].plot(x, reference_df["Raw_temperature"].to_numpy(), color="black", linewidth=1.2, label="True Temp")
    for exp in experiments:
        exp_df = local_df[local_df["experiment_label"] == exp].sort_values("Global_Pos")
        axes[3].plot(exp_df["Global_Pos"], exp_df["Recon_3"], linewidth=1.1, label=f"{exp} Recon")
    axes[3].set_ylabel("Temperature")
    axes[3].set_xlabel("Global position")
...
def plot_bms_case(df, output_path, title, entity_name, center_pos, radius):
    ...
    for exp in experiments:
        exp_df = local_df[local_df["experiment_label"] == exp].sort_values("Global_Pos")
        if exp in {"C3+Physics", "C4+Physics", "C4-StructureOnly"}:
            axes[1].plot(exp_df["Global_Pos"], exp_df["A_Score_Global"], linewidth=1.5, label=f"{exp} Global")
    c4_df = local_df[local_df["experiment_label"] == "C4+Physics"].sort_values("Global_Pos")
    if not c4_df.empty:
        axes[1].plot(c4_df["Global_Pos"], c4_df["A_Score_Main_Branch"], linewidth=1.2, linestyle="--", label="C4+Physics Main")
        axes[1].plot(c4_df["Global_Pos"], c4_df["A_Score_Residual_Branch"], linewidth=1.2, linestyle=":", label="C4+Physics Residual")
    axes[1].set_ylabel("Score")
    axes[1].grid(alpha=0.25)
    axes[1].legend(frameon=False, ncol=2)

    axes[2].plot(x, reference_df["Raw_BMSnVmean"].to_numpy(), color="black", linewidth=1.2, label="True Voltage")
    for exp in experiments:
        if exp not in {"C3+Physics", "C4+Physics", "C4-StructureOnly"}:
            continue
        exp_df = local_df[local_df["experiment_label"] == exp].sort_values("Global_Pos")
        axes[2].plot(exp_df["Global_Pos"], exp_df[f"Recon_{voltage_idx}"], linewidth=1.1, label=f"{exp} Recon")
    axes[2].set_ylabel("Voltage")
    axes[2].grid(alpha=0.25)
    axes[2].legend(frameon=False, ncol=2)

    axes[3].plot(x, reference_df["Raw_BMSnTmean"].to_numpy(), color="black", linewidth=1.2, label="True Temp")
    for exp in experiments:
        if exp not in {"C3+Physics", "C4+Physics", "C4-StructureOnly"}:
            continue
        exp_df = local_df[local_df["experiment_label"] == exp].sort_values("Global_Pos")
        axes[3].plot(exp_df["Global_Pos"], exp_df[f"Recon_{temperature_idx}"], linewidth=1.1, label=f"{exp} Recon")
    axes[3].set_ylabel("Temperature")
    axes[3].set_xlabel("Global position")
    axes[3].grid(alpha=0.25)
    axes[3].legend(frameon=False, ncol=2)
...
def analyze_nasa(experiments, output_dir):
    ...
    plot_slice_boxpanels(
        ...
        "NASA phase-aware score distribution",
    )
    plot_grouped_metric(
        overall_summary,
        nasa_dir / "nasa_phase_error_summary.png",
        "NASA phase-aware reconstruction error summary",
...
def analyze_bms(experiments, output_dir):
    ...
    plot_slice_boxpanels(
        ...
        "BMS regulation-aware score distribution",
    )
    plot_grouped_metric(
        overall_summary,
        bms_dir / "bms_regulation_error_summary.png",
        "BMS regulation-aware reconstruction error summary",
        "Recon_Error_Global_mean",
        BMS_SLICE_ORDER,
    )
...
def build_manifest(output_dir, registries, nasa_result, bms_result):
    manifest = {
...
def main():
    ...
    experiments = collect_experiments(registry_paths)
    nasa_result = analyze_nasa(experiments, output_dir)
    bms_result = analyze_bms(experiments, output_dir)
    build_manifest(output_dir, registry_paths, nasa_result, bms_result)
